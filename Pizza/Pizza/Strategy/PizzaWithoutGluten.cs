...
{
   public class PizzaWithoutGluten: IBuildPizzaStrategy
    {
        private IPizzaBuildState _pizzaBuildState;
        private readonly PizzaBuilder _builder;
        public PizzaWithoutGluten(PizzaBuilder builder, IPizzaBuildState pizzaBuildState)
        {
            _builder = builder;
        ...
        public void BuildPizza(string pizzaName)
        {
            ...
            listLogs.Add($"{DateTime.Now}, {this.GetType().Name}: {pizzaName} - Create New Pizza Without Gluten");
            listLogs.Add($"{DateTime.Now}, {this.GetType().Name}: {pizzaName} - State: In Progress");
            _builder.Name();
            _builder.AddDough();
